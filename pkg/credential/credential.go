...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via ONEAPPFACTORY_KEY_PASSPHRASE
...
//  2. ONEAPPFACTORY_SSH_KEY_PATH env var
//  3. ~/.ssh/oneappfactory_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "ONEAPPFACTORY_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading ONEAPPFACTORY_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "ONEAPPFACTORY_SSH_KEY_PATH"

// oneAppFactoryHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const oneAppFactoryHome = "ONEAPPFACTORY_HOME"

const (
	fileScheme = "file://"
	encScheme  = "enc://"
	hkdfInfo   = "oneappfactory-credential-v1"
	saltLen    = 16
	nonceLen   = 12
	keyLen     = 32
)
...
//
// passphrase is required (ONEAPPFACTORY_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// ONEAPPFACTORY_SSH_KEY_PATH env var or ~/.ssh/oneappfactory_ed25519.key.
...
//   - exact match with ONEAPPFACTORY_SSH_KEY_PATH env var
//   - within the ONEAPPFACTORY_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with ONEAPPFACTORY_SSH_KEY_PATH.
	...
	// Within ONEAPPFACTORY_HOME.
	if oneAppFactoryHomePath := os.Getenv(oneAppFactoryHome); oneAppFactoryHomePath != "" {
		if isWithinDir(clean, oneAppFactoryHomePath) {
			return true
		}
	}
...
// Final key: HKDF-SHA256(ikm, salt, info="oneappfactory-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set ONEAPPFACTORY_SSH_KEY_PATH or place key at ~/.ssh/oneappfactory_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (ONEAPPFACTORY_SSH_KEY_PATH, ONEAPPFACTORY_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. ONEAPPFACTORY_SSH_KEY_PATH env var
//  3. ~/.ssh/oneappfactory_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the OneAppFactory-specific SSH key path if it exists.
