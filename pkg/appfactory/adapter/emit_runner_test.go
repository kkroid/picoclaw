...
func TestTryDeterministicEmitGenericSchemaSurfacesUseDomainFields(t *testing.T) {
	...
	assertFileMarkers("lib/repositories/record_repository.dart", []string{"Future<List<CourseAssignment>> loadRecords()", "Future<CourseAssignmentSummary> loadSummary();", "record.assignmentId", "return CourseAssignmentSummary(", "dueSoonCount: records.where", "overdueCount: records.where", "doneCount: records.where"}, []string{"record.recordId"})
	assertFileMarkers("lib/views/home_page.dart", []string{"r.assignmentTitle", "controller.summary.dueSoonCount.toString()", "openLiteCopy.summaryDueSoonCountLabel"}, []string{"RecordStatus.done", "r.title"})
	assertFileMarkers("lib/controllers/home_controller.dart", []string{"CourseAssignmentSummary _summary = const CourseAssignmentSummary();", "CourseAssignmentSummary get summary => _summary;", "_summary = await _repository.loadSummary();"}, []string{"DashboardSummary _summary"})
	assertFileMarkers("lib/template/open_lite_copy.dart", []string{"String get titleFieldLabel => '作业标题';", "String get categoryFieldLabel => '课程名称';", "String get dateFieldLabel => '截止日期';", "String get detailStatusLabel => '作业状态';", "String get summaryDueSoonCountLabel => 'due_soon_count';"}, []string{"Open Lite Seed"})
	...
	assertFileMarkers("test/widget_test.dart", []string{"import 'package:flutter/foundation.dart';", "import 'package:flutter_open_lite/models/record.dart';", "InMemoryRecordRepository(seedRecords: [CourseAssignment(assignmentTitle: '测试记录', courseName: '测试分类', note: '测试备注')])", "open lite app supports schema-driven create and inspect flow", "expect(find.textContaining(openLiteCopy.summaryDueSoonCountLabel), findsWidgets);", "await tester.enterText(find.byKey(const Key('secondary-field')), '测试分类');", "await tester.enterText(find.byKey(const Key('note-field')), '测试备注');", "expect(find.text('测试备注'), findsOneWidget);"}, []string{"InMemoryRecordRepository();"})
}
