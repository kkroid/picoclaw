...
func compileBookkeepingSpec(request Request, requirementText string) domainSpec {
	...
	return domainSpec{
		...
		ProblemStatement:        "当前实验阶段需要一条从自然语言需求到 Builder 输入包的最小闭环，用来验证 OneAppFactory 能否把简单记账需求整理成结构化规格，并围绕中性交互承载驱动 Builder 执行最小任务链。",
...
func defaultFlutterFallbackChecks() []appruns.AcceptanceCheck {
	return []appruns.AcceptanceCheck{{CheckID: "check-structural-template-files-ready", Label: "确认 Flutter seed 工作区存在", Stage: "baseline", Required: true, Commands: []string{"grep -q . pubspec.yaml lib/main.dart test/widget_test.dart"}, SuccessCriteria: "默认 seed 工作区关键文件已就位。", TimeoutSeconds: 30}, {CheckID: "check-oneappfactory-thin-fallback-probe", Label: "确认默认 thin fallback 写入探针", Stage: "cheap", Required: true, Commands: []string{"grep -E \"Generated by OneAppFactory thin executor.|'goalSummary':\" lib/oneappfactory_executor_probe.dart >/dev/null 2>&1"}, SuccessCriteria: "默认 thin executor fallback 已在允许目录中写入探针文件，用于证明 OneAppFactory fallback 链可用。", TimeoutSeconds: 30}, {CheckID: "check-oneappfactory-thin-fallback-metadata", Label: "确认 fallback 探针包含执行元数据", Stage: "cheap", Required: true, Commands: []string{"grep -E \"'taskCount': [0-9]+,|'acceptanceCheckCount': [0-9]+,\" lib/oneappfactory_executor_probe.dart >/dev/null 2>&1"}, SuccessCriteria: "fallback 探针里已带上 task 与 acceptance check 元数据，供定位 OneAppFactory fallback 输入边界，不代表真实 builder-runtime 产出。", TimeoutSeconds: 30}}
}
