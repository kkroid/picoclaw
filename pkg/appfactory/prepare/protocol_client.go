...
func protocolRuntimeBootstrapCheckCommand() string {
	return "grep -E 'http:|web_socket_channel:|flutter_markdown:|shared_preferences:|provider:' pubspec.yaml >/dev/null 2>&1 && grep -ER 'OnePilotApp|MultiProvider|ConnectionProvider|ProjectProvider|ConversationProvider|FileProvider' lib/main.dart lib/app.dart lib/providers >/dev/null 2>&1 && ! grep -ER 'Flutter Demo Home Page|You have pushed the button this many times|Counter increments smoke test|_counter|_incrementCounter|MyHomePage' lib test >/dev/null 2>&1"
}
...
func protocolClientAllowedPaths() []string {
	return []string{"lib/**", "assets/**", "pubspec.yaml", "test/**", "android/app/build.gradle.kts", "android/app/src/main/AndroidManifest.xml", "android/app/src/main/res/values/strings.xml"}
}

func protocolClientProtectedPaths() []string {
	return []string{"android/app/src/main/kotlin/**", "android/gradle/**", "android/local.properties", "ios/**", "linux/**", "macos/**", "windows/**"}
}
