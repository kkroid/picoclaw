...
func TestRelationRichPrepareSampleMatchesExampleBundle(t *testing.T) {
	...
	bundle := compileRelationRichPrepareSampleBundle(t)
...
func compileRelationRichPrepareSampleBundle(t *testing.T) Bundle {
	t.Helper()
	requestPath := filepath.Join("..", "..", "..", "examples", "appfactory", "relation-rich", "project-task-tag", "compile-request.sample.json")
	...
	bundle, err := Compile(Request{
		...
		Now: func() time.Time {
			return time.Date(2026, 4, 10, 16, 0, 0, 0, time.UTC)
		},
	})
	if err != nil {
