...
func defaultTemplateRegistry() []TemplateRegistryEntry {
	return []TemplateRegistryEntry{
		...
		},
		{
			...
			Name:           "Flutter Open Lite",
			Description:    "适合作为通用工具类 Android MVP 的多页面离线模板。",
			PinnedRef:      "v0.1.0",
			License:        "MIT",
			Stack:          "flutter",
			AndroidSupport: true,
			Capabilities:   []string{"navigation", "summary-card", "list", "form", "detail", "local-storage", "theme"},
			HealthStatus:   "healthy",
			RiskNotes:      []string{"模板只冻结最小通用页面骨架，真实业务字段与流程仍需由 PRD 和 task bundle 二次裁剪。"},
			SelectionHints: []string{"适合需要首页摘要、记录列表、表单编辑、详情页和本地持久化的单任务工具类 MVP。"},
		},
		{
