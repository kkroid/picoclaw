...
- [ ] 先扩 `pkg/appfactory/emitter/app_entry_emitter.go`，让 generic app entry 进入 deterministic 主路径。
- [ ] 再扩 `pkg/appfactory/emitter/overview_emitter.go`，让 generic overview surface 可 emit。
- [ ] 将 generic list controller/page 的 runtime canonical helper 提升为正式 emitter 或 canonical factory。
- [ ] 扩 `pkg/appfactory/emitter/mutation_emitter.go` 与 `pkg/appfactory/emitter/inspection_emitter.go`，补齐 generic mutation 与 inspection。
- [ ] 统一 `emit_runner` 的 generic 路由，确保 generic 先走 deterministic，再落到 model fallback。

P6 退出标准：generic 的 app entry、overview、collection、mutation、inspection 五类关键表面不再主要依赖 analyze repair 多文件自救。

#### P6 承接拆分：由 registry 过渡到 deterministic emit

- [ ] P6.1 让 `app_entry` / `surface-overview` 的 emitter 或 canonical factory 优先消费 registry snapshot，而不是再次自行猜 `HomePage` / `HomeController` / `home_page.dart`。
- [ ] P6.2 让 collection / mutation / inspection 的 emitter 与当前 runtime canonical factory 共用同一份 registry consumer，实现“先 canonical factory，后 deterministic emitter”的无缝切换，而不是两套 path/class 推断并存。
- [ ] P6.3 把 widget test 输出也纳入同一条 registry 驱动路径：已覆盖的表面走 deterministic emit 或 canonical factory，未覆盖的才保留 template-private fallback repair。

### P7：收口 validation 与 repair target 提取

- [ ] 重写 direct-failure target 提取，让 repair slice 在直接失败文件之外，还能继承原 task 的 `binding_refs`、`surface_refs` 与路径类别。
- [ ] 重写 helper preservation guard，区分跨文件仍被引用的 helper、同文件临时 shim 与模板私有 canonical helper。
- [ ] 收口 semantic guard 与 allowed paths / owned paths 判断，避免旧 topology、旧 `screen_ref` 或历史 helper 名继续放大 repair target。

P7 退出标准：analyze/test/closure repair 的 target 选择与保护边界由 policy 驱动，而不是由历史样本残留驱动。

#### P7 承接拆分：repair target 与 helper guard 的 registry 化

- [ ] P7.1 让 direct-failure target 提取在直接失败文件之外，继续继承当前 task 的 `binding_refs`、`surface_refs`、`path_classes` 与可选 `registry_keys`，避免 repair slice 重新被默认文件名放大。
- [ ] P7.2 重写 helper preservation guard，让它优先依据 registry ownership 区分“仍归当前表面持有的 canonical helper”“同文件临时 shim”“已经游离的 stale import / helper”，不再只按旧 helper 名硬保留。
- [ ] P7.3 把 failure signature registry 与 runtime shell marker、`latest.md`、notification / status facade 对齐到同一套 `failure_class + binding/surface + path_class` 语义，减少同一失败在不同层的重复命名。

### P8：重建 strict `/jobs` generic 回归矩阵

- [ ] 改 `scripts/run-appfactory-jobs-regression.sh`，把默认入口从单一样本 gate 提升为 generic matrix。
- [ ] 至少补齐 3 到 4 个低重叠 generic fixture，覆盖经典 list-detail-form、no-home-no-detail、no-filter 或 summary 简化等拓扑。
- [ ] 统一 latest 输出结构，让每条样本都暴露 `job_status`、`script_status`、`manual_equivalence`、`probe_only`、frontier、artifact 路径与真实失败类别。
- [ ] 保留 bookkeeping 与 relation-rich completed 样本，但它们只做防回退护栏，不再覆盖 generic 主结论。

P8 退出标准：strict `/jobs` generic 主线从“单一样本过线”升级为“generic capability matrix 过线”。

### P9：冻结新门禁规则

- [ ] 规定后续 generic 修复只允许三种落点：deterministic emitter 扩展、template repair policy、validation/repair policy。
- [ ] 新增 live 修复如不能提升为上述三类之一，直接拒绝进入主线。
- [ ] 在 code review 与设计文档中加入审核问题单：是否使用了样本文件名作为公共语义，是否把模板私有路径提升成公共触发器，是否能够被 taxonomy 与 policy registry 表达。

P9 退出标准：generic 主线后续不再回到“边跑 live 样本边追加症状补丁”的工作方式。
