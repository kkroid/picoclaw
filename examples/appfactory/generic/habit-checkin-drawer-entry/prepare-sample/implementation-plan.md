...
- Job ID: job-habit-checkin-drawer-entry-sample-001
- PRD ID: prd-habit-checkin-drawer-entry-sample-001

## 阶段拆分

- 阶段 1：冻结习惯记录和习惯概览摘要和本地仓储边界。
- 阶段 2：搭建概览、集合浏览、实体变更和结果检查承载单元并接通最小导航。
- 阶段 3：补齐新增录入、集合浏览、结果检查和状态筛选主流程，为后续真实 validate 链路预留稳定输入包。
...
- 完成标准：集合浏览控制器可提供领域记录集合；集合浏览入口可浏览领域记录；结果检查入口与集合数据保持一致；集合浏览入口可按领域状态筛选记录
...
- 目标：根据集合浏览控制器和领域文案绑定集合浏览承载单元，提供记录浏览、筛选和结果定位入口。
- 分类：screen
- 依赖任务：task-create-list-controller, task-create-copy
- 目标路径：lib/views/record_list_page.dart
- 完成标准：集合浏览承载单元可浏览领域记录；结果检查入口与集合数据保持一致；集合结果可按领域状态筛选
...
- 完成标准：结果检查承载单元可展示单条领域记录；编辑入口已预留
