...
- Job ID: job-weight-tracker-detail-without-delete-sample-001
- PRD ID: prd-weight-tracker-detail-without-delete-sample-001

## 阶段拆分

- 阶段 1：冻结体重记录和本地仓储边界。
- 阶段 2：搭建集合浏览和结果检查承载单元并接通最小导航。
- 阶段 3：补齐集合浏览和结果检查主流程，为后续真实 validate 链路预留稳定输入包。
...
- 目标：根据集合浏览控制器和领域文案绑定集合浏览承载单元，提供记录浏览和结果定位入口。
- 分类：screen
...
- 完成标准：结果检查承载单元可展示单条领域记录

### 接线应用入口与承载单元路由

- 目标：把已创建的控制器、承载单元和仓储接线到应用入口，形成稳定的中性交互骨架。
- 分类：screen
- 依赖任务：task-bind-collection-surface, task-bind-inspection-surface, task-create-list-controller, task-create-repository
- 目标路径：lib/main.dart
- 完成标准：应用入口完成接线；集合浏览、结果检查承载单元导航可达
...
- 阻断 open-lite 默认 counter 模板：stage=cheap，commands=grep -q . lib/main.dart lib/models/record.dart lib/views/record_list_page.dart lib/controllers/record_list_controller.dart lib/views/record_detail_page.dart lib/repositories/record_repository.dart && ! grep -E 'Flutter Demo Home Page|You have pushed the button this many times|Counter increments smoke test|_counter|_incrementCounter|MyHomePage' lib/main.dart test/widget_test.dart >/dev/null 2>&1
- 确认 open-lite CRUD 页面与控制器已接线：stage=cheap，commands=grep -E 'RecordListPage' lib/main.dart >/dev/null 2>&1 && grep -E 'RecordListPage' lib/views/record_list_page.dart >/dev/null 2>&1 && grep -E 'RecordDetailPage' lib/main.dart lib/views/record_detail_page.dart lib/views/record_list_page.dart >/dev/null 2>&1
